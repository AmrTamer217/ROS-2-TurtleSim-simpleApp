...
class ShapeNode(Node):
        ...
        while rclpy.ok():
            try:
                ...
                if choice == '1':
                    ...
                    self.send_shape_command('star')
                    print("Drawing star... Please wait.")
                elif choice == '4':
                    self.send_shape_command('clear')
                elif choice == '5':
                    self.send_shape_command('stop')
                    self.get_logger().info('Stopping turtle...')
                    print("Turtle stopped. You can continue selecting shapes or press Ctrl+C to exit.")
                    # Don't break here, allow continued operation
                else:
    ...
    def display_menu(self):
        ...
        print("3. ⭐ 5-Pointed Star")
        print("4. 🧹 Clear Screen")
        print("5. 🛑 Stop Current Drawing")
    ...
    def send_shape_command(self, shape):
        ...
        if shape != 'stop':
            self.get_logger().info(f'Sending command: {shape}')
        else:
